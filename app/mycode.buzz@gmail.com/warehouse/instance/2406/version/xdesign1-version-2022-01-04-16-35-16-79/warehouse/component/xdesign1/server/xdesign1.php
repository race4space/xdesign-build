...
class XDesign{
  function __construct() {
    ...
    function fn_setPath(){ 
      ...
      $this->str_name_folder_root="root";      
    ...
    function fn_buildProject($obj_ini){
      ...
      $str_path_source=$this->fn_getfolderpathInstance($int_idRecord);        
      $this->fn_transferFolder($this->str_name_folder_warehouse, $str_path_source, $str_path_folder);
      $this->fn_transferFolderContent($this->str_name_folder_root, $str_path_source, $str_path_folder);      
...
function fn_transferFolder($str_name_folder, $str_path_source_parent, $str_path_destination_parent){
...
function fn_transferFolderContent($str_name_folder, $str_path_source_parent, $str_path_destination_parent){
  
  $str_path_source=$str_path_source_parent."/".$str_name_folder;
  $bln_fileexist=file_exists($str_path_source);                   
  if($bln_fileexist){                    
    $str_path_destination=$str_path_destination_parent;
    $this->fn_copyFolderContent($str_path_source, $str_path_destination);
  }  
}  


function fn_copyFolder($source, $dest, $bln_replace=true){

  $this->fn_addEcho("source: ".$source);
  $this->fn_addEcho("dest: ".$dest);


  $bln_exist=file_exists($source);
  if(!$bln_exist){
    return;
  }

  $bln_exist=file_exists($dest);
  if($bln_exist && $bln_replace){
    $this->fn_deleteFile($dest);    
  }

  $this->fn_createFolder($dest);      
  
  $this->fn_copyFolderContent($source, $dest);
